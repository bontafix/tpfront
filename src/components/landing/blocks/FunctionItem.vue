<template>
  <div :class="['function-item', reverse ? 'function-item--reversed' : '']">
    <div class="function-item__mobile-image-container--mobile">
      <img 
        loading="lazy" 
        class="function-item__media function-item__media--mobile" 
        :src="getImageUrl(mobilePath)" 
        :alt="`${title} – мобильная версия`" 
      />
    </div>
    <img 
      loading="lazy" 
      class="function-item__media function-item__media--tablet" 
      :src="getImageUrl(imagePath)" 
      :alt="`${title} – планшетная версия`" 
    />
    <img 
      v-if="reverse" 
      loading="lazy" 
      class="function-item__media function-item__media--main-reversed" 
      :src="getImageUrl(imagePath)" 
    ...
    <div :class="['function-item__info', reverse ? 'function-item__info--reversed' : '']">
      <div class="function-item__icon">
        <img loading="lazy" class="icon" :src="getIconUrl(iconPath)" :alt="`${title} иконка`" />
      </div>
    ...
    <img 
      v-if="!reverse" 
      loading="lazy" 
      class="function-item__media function-item__media--main" 
      :src="getImageUrl(imagePath)" 
      :alt="`${title} – основное изображение`" 
    />
  </div>
