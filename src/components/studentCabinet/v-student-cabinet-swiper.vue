  ...
  <div class="v-student-cabinet-swiper">
    <div class="flex gap-3 items-center v-student-cabinet-swiper__buttons">
      <template v-for="type in ['rotate-180', '']" :key="type">
        <div :class="`${type} custom-swiper-button day-el student-swiper-button-${type.length ? 'prev' : 'next'}`">
          <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M15.7578 20.0001H24.2427M24.2427 20.0001L20.0003 15.7577M24.2427 20.0001L20.0003 24.2425"
              stroke="#717680" stroke-width="1.85556" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </div>
        <div :class="`${type} custom-swiper-button night-el student-swiper-button-${type.length ? 'prev' : 'next'}`">
          <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect x="40" width="40" height="40" rx="20" transform="rotate(90 40 0)" fill="white" fill-opacity="0.08" />
            <path d="M15.7573 20.0001H24.2422M24.2422 20.0001L19.9998 15.7577M24.2422 20.0001L19.9998 24.2425"
              stroke="#E9EAEB" stroke-width="1.85556" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </div>
      </template>
    </div>
    ...
    <div v-if="debug" class="debug-info mb-4 p-2 bg-gray-100 rounded">
      ...
      <button @click="() => clearCache(props.profile?.id)" class="mt-2 px-2 py-1 bg-red-500 text-white text-xs rounded">
        Clear Cache
      </button>
      <button @click="refreshData" class="mt-2 ml-2 px-2 py-1 bg-blue-500 text-white text-xs rounded">
        Refresh
      </button>
    </div>
    ...
    <template v-else>
      <swiper class="v-student-cabinet-swiper__prev-row" ref="swiperRef" @swiper="onSwiper"
        @slide-change="onSlideChange" v-bind="swiperOptions" :modules="modules">
        <swiper-slide class="v-student-cabinet-swiper__prev-item lesson-swiper" v-for="lesson in lessons"
          :key="lesson.id">
          <div class="lesson-swiper__content">
            ...
            <div class="lesson-swiper__info">
              <div class="lesson-swiper__info-row">
                ...
                <div class="lesson-swiper__block">
                  <template v-if="lesson.topics && lesson.topics.length">
                    <div class="lesson-swiper__theme theme" @click="() => openModal('themes', lesson.id)"
                      v-for="theme in lesson.topics" :key="theme.id">
              ...
              <div class="lesson-swiper__info-row">
                ...
                <div class="lesson-swiper__block lesson-swiper__col start">
                  <template v-if="lesson.problems && lesson.problems.length">
                    <p v-for="problem in lesson.problems" :key="problem.id || problem" class="lesson-swiper__subtitle">
              ...
              <div class="lesson-swiper__info-row">
                ...
                <div class="lesson-swiper__block col" v-if="lesson.homeworks && lesson.homeworks.length">
                  <p class="lesson-swiper__subtitle">{{ lesson.homeworks[0].title }}</p>
                  <p class="lesson-swiper__subtitle" :class="getDeadlineClass(lesson.homeworks[0].due_date)">
                    Дедлайн {{ formatDate(lesson.homeworks[0].due_date) }}
                  </p>
                  <div class="lesson-swiper__homework">
                    <v-files-handler :model-value="transformedFiles(lesson.homeworks[0].files)" :deletable="false" />
                    <button class="lesson-swiper__button custom-btn blue" @click="loadAnswer(lesson.homeworks[0].id)"
                      v-if="!getStudentFilesAnswer(lesson).length" й>
              ...
              <div class="lesson-swiper__info-row" v-if="getStudentFilesAnswer(lesson).length">
                ...
                <div class="lesson-swiper__block lesson-swiper__answer nowrap">
                  <v-files-handler :model-value="getStudentFilesAnswer(lesson)" :deletable="false" />
                  <img src="/src/assets/images/pen.svg" alt="" @click="editHomework(lesson.homeworks[0].id)" />
                </div>
              </div>
...
function getStudentFilesAnswer(lesson) {
  if (lesson.homeworks && lesson.homeworks[0]) {
    ...
    if (
      ...
      const result = homework.student_answers.map((item) => {
        return {
          id: item.id,
          name: item.file_name,
          size: item.file_size,
